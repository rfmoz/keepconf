...
for fcsv in ${dkeep}*.csv; do
	...
	{ echo -n "keepconf_ok{file=\"$fbase\"} " && grep OK $fcsv | awk -F "\"*\"*" '{print $4}'
	  echo -n "keepconf_bad{file=\"$fbase\"} " && grep BAD $fcsv | awk -F "\"*\"*" '{print $4}'
	  echo -n "keepconf_fetch_time{file=\"$fbase\"} " && grep FETCH-T $fcsv | awk -F "\"*\"*" '{print $4}'
	  echo -n "keepconf_commit_time{file=\"$fbase\"} " && grep COMMIT-T $fcsv | awk -F "\"*\"*" '{print $4}'
	  echo -n "keepconf_total_time{file=\"$fbase\"} " && grep TOTAL-T $fcsv | awk -F "\"*\"*" '{print $4}'
	} >> $dfile
done
